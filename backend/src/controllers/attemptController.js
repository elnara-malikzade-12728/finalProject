...
async function startTestAttempt(req, res, next) {
    try {
        ...
        if (test.type === "LESSON" && test.lessonId) {
            ...
            const isFreePreview = await isFreePreviewLesson(lesson.module.courseId, test.lessonId);
            if (!isFreePreview && !(await canAccessCourse(req.user.id, lesson.module.courseId))) {
