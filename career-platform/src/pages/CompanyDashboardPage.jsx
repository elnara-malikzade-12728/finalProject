...
function CompanyDashboardPage() {
  ...
  return <section className="section"><div className="container admin-page">
    ...
    <form className="simple-card" onSubmit={async (event) => { event.preventDefault(); await run(() => saveCompany(company), "Şirkət profili saxlanıldı."); }}>
    ...
    {data && <>
      <div className="simple-card-grid">{Object.entries({ Əməkdaşlar: data.stats.employees, "Aktiv abunəlik": data.stats.activeSubscriptions, Qeydiyyatlar: data.stats.enrollments, Sertifikatlar: data.stats.certificates, "Prioritet vakansiyalar": data.stats.priorityJobs }).map(([label, value]) => <article className="simple-card" key={label}><h3>{value}</h3><p>{label}</p></article>)}</div>
      <div className="course-admin-forms">
        <section className="simple-card"><h2>Əməkdaşlar</h2><form className="course-admin-inline" onSubmit={async (e) => { e.preventDefault(); if (await run(() => addCompanyEmployee(email), "Əməkdaş əlavə edildi.")) setEmail(""); }}><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Synex hesabının e-poçtu" required /><button className="button button-primary"><Plus size={16} /> Əlavə et</button></form>{data.members.map((member) => <p key={member.id}>{member.name} · {member.email} · {member.certificates} sertifikat {member.userId !== data.company.ownerId && <button type="button" onClick={() => run(() => removeCompanyEmployee(member.id), "Əməkdaş silindi.")}><Trash2 size={14} /></button>}</p>)}</section>
        <form className="simple-card" onSubmit={async (e) => { e.preventDefault(); if (await run(() => createCompanyJob(job), "Prioritet vakansiya yaradıldı.")) setJob({ title: "", description: "", location: "", employmentType: "FULL_TIME", experienceLevel: "JUNIOR" }); }}><h2>Prioritet vakansiya</h2><input value={job.title} onChange={(e) => setJob({ ...job, title: e.target.value })} placeholder="Vakansiya adı" required /><textarea value={job.description} onChange={(e) => setJob({ ...job, description: e.target.value })} placeholder="Təsvir və tələblər" required /><input value={job.location} onChange={(e) => setJob({ ...job, location: e.target.value })} placeholder="Məkan" /><select value={job.employmentType} onChange={(e) => setJob({ ...job, employmentType: e.target.value })}><option value="FULL_TIME">Tam iş günü</option><option value="PART_TIME">Yarım iş günü</option><option value="REMOTE">Uzaqdan</option><option value="INTERNSHIP">Təcrübə</option><option value="FREELANCE">Frilans</option></select><button className="button button-primary">Vakansiyanı yerləşdir</button></form>
      </div>
    </>}
  </div></section>;
}
