  ...
  return <section className="section"><div className="container admin-page company-dashboard">
    ...
    {data && <>
      ...
      <div className="company-workspace">
        <section className="simple-card company-panel-card"><h2>Əməkdaşlar</h2><form className="company-employee-form" onSubmit={async (e) => { e.preventDefault(); if (await run(() => addCompanyEmployee(email), "Hesab mövcuddursa, əməkdaşlıq dəvəti göndərildi.")) setEmail(""); }}><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Synex hesabının e-poçtu" required /><button className="button button-primary"><Plus size={16} /> Əlavə et</button></form><div className="company-member-list">{data.members.map((member) => <p key={member.id}><span>{member.name}<small>{member.email} · {member.certificates} sertifikat</small></span>{member.userId !== data.company.ownerId && <button type="button" aria-label="Əməkdaşı sil" onClick={() => run(() => removeCompanyEmployee(member.id), "Əməkdaş silindi.")}><Trash2 size={16} /></button>}</p>)}</div></section>
